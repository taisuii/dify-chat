...
**版本说明**：当前 @dify-chat/* 0.7.7、@dify-chat/widget 0.1.8。v0.1.7 侧边栏自适应、聊天区高度与滚动；v0.1.8 恢复 containerMinHeight 默认 calc 保证流式打字效果、侧边栏最小宽度 188px。若 Next.js 构建仍因 CJS context require 报错，可显式使用：`import { DifyChat } from '@dify-chat/widget/dist/index.js'`。
